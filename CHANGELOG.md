...
- **`ClaudeCode.Adapter.Codex`** — Experimental sibling adapter that drives OpenAI's `codex` CLI (`codex app-server --transport stdio`) as a backend instead of `claude`. Translates codex's JSON-RPC 2.0 notifications (`agentMessageDelta`, `reasoningTextDelta`, `itemCompleted`, `turnCompleted`, etc.) into `ClaudeCode.Message.*` structs in-process, so streams, typed messages, and `ClaudeCode.Message.ResultMessage` auto-completion continue to work unchanged. Minimum-viable: streaming text + reasoning, command/MCP tool visibility, turn boundaries, and `interrupt/1`. `can_use_tool`/hooks/MCP config translation are intentionally out of scope for this first cut. Configure via `adapter: {ClaudeCode.Adapter.Codex, codex_path: "codex", model: "gpt-5-codex"}`.
